...
extern void loadUsrProgram(int addr, int len, int head, int cylinder, int sector);
        ...
        if(strcmp(userbuff, commands[Shutdown]) == 0) {
            ...
            if(valid){
                for (int i = 0, len = strlen(target); i < len; i++)
                {
                    ...
                    loadUsrProgram(Comlist[pid].addr, Comlist[pid].len, Comlist[pid].head, Comlist[pid].cylinder, Comlist[pid].sector);
                    cls();
                }
            }
            else{
